...
void resolve_path_display(char* resolved_path, const char* path) {
    ...
    if (path[0] == '/') { // Absolute path
        ...
        strcpy(resolved_path, path); // For absolute path, resolved is just the path
        return;
    } else { // Relative path
    ...
    } else if (strlen(normalized_path) > 1) {
        strcpy(resolved_path, normalized_path); // Use allowed strcpy
    } else {
        strcpy(resolved_path, "/"); // Default to root if something goes wrong
    }
}
...
void handle_cd(const char* path, int current_row) {
    ...
    (void)current_row; // Suppress unused parameter warning
}
