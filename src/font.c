#include "header/driver/font.h"

// Data font 5x8 (minimal set)
// Bit 76543... (5 bit paling kiri digunakan untuk font 5xN)
const unsigned char font5x8[128][FONT_HEIGHT] = {
    // Isi dengan data font. Untuk karakter yang tidak ada, bisa diisi dengan blok atau '?'
    // Contoh untuk beberapa karakter:
    [' '] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
    ['A'] = {0x70,0x88,0x88,0xF8,0x88,0x88,0x88,0x00}, // 0b01110000, 0b10001000, ...
    ['B'] = {0xF0,0x88,0xF0,0x88,0x88,0x88,0xF0,0x00},
    ['C'] = {0x70,0x80,0x80,0x80,0x80,0x80,0x70,0x00},
    ['D'] = {0xE0,0x88,0x88,0x88,0x88,0x88,0xE0,0x00},
    ['E'] = {0xF8,0x80,0x80,0xF0,0x80,0x80,0xF8,0x00},
    ['F'] = {0xF8,0x80,0x80,0xF0,0x80,0x80,0x80,0x00},
    ['G'] = {0x70,0x80,0x80,0x98,0x88,0x88,0x78,0x00},
    ['H'] = {0x88,0x88,0x88,0xF8,0x88,0x88,0x88,0x00},
    ['I'] = {0x70,0x20,0x20,0x20,0x20,0x20,0x70,0x00},
    ['J'] = {0x18,0x08,0x08,0x08,0x08,0x88,0x70,0x00},
    ['K'] = {0x88,0x90,0xA0,0xC0,0xA0,0x90,0x88,0x00},
    ['L'] = {0x80,0x80,0x80,0x80,0x80,0x80,0xF8,0x00},
    ['M'] = {0x88,0xD8,0xA8,0x88,0x88,0x88,0x88,0x00},
    ['N'] = {0x88,0xC8,0xA8,0x98,0x88,0x88,0x88,0x00},
    ['O'] = {0x70,0x88,0x88,0x88,0x88,0x88,0x70,0x00},
    ['P'] = {0xF0,0x88,0x88,0xF0,0x80,0x80,0x80,0x00},
    ['Q'] = {0x70,0x88,0x88,0x88,0xA8,0x90,0x78,0x00},
    ['R'] = {0xF0,0x88,0x88,0xF0,0xA0,0x90,0x88,0x00},
    ['S'] = {0x78,0x80,0x80,0x70,0x08,0x08,0xF0,0x00},
    ['T'] = {0xF8,0x20,0x20,0x20,0x20,0x20,0x20,0x00},
    ['U'] = {0x88,0x88,0x88,0x88,0x88,0x88,0x78,0x00},
    ['V'] = {0x88,0x88,0x88,0x88,0x88,0x50,0x20,0x00},
    ['W'] = {0x88,0x88,0x88,0xA8,0xA8,0xD8,0x88,0x00},
    ['X'] = {0x88,0x88,0x50,0x20,0x50,0x88,0x88,0x00},
    ['Y'] = {0x88,0x88,0x50,0x20,0x20,0x20,0x20,0x00},
    ['Z'] = {0xF8,0x08,0x10,0x20,0x40,0x80,0xF8,0x00},
    ['0'] = {0x70,0x88,0x98,0xA8,0xC8,0x88,0x70,0x00},
    ['1'] = {0x20,0x60,0x20,0x20,0x20,0x20,0x70,0x00},
    ['2'] = {0x70,0x88,0x08,0x70,0x80,0x80,0xF8,0x00},
    ['3'] = {0xF8,0x08,0x08,0x70,0x08,0x08,0xF8,0x00},
    ['4'] = {0x80,0x80,0x88,0x98,0xF8,0x08,0x08,0x00},
    ['5'] = {0xF8,0x80,0x80,0xF0,0x08,0x08,0xF0,0x00},
    ['6'] = {0x70,0x80,0x80,0xF0,0x88,0x88,0x70,0x00},
    ['7'] = {0xF8,0x08,0x08,0x10,0x20,0x20,0x20,0x00},
    ['8'] = {0x70,0x88,0x88,0x70,0x88,0x88,0x70,0x00},
    ['9'] = {0x70,0x88,0x88,0x78,0x08,0x08,0x70,0x00},
    ['.'] = {0x00,0x00,0x00,0x00,0x00,0x00,0x60,0x60},
    [':'] = {0x00,0x00,0x60,0x00,0x60,0x00,0x00,0x00},
    ['/'] = {0x08,0x08,0x10,0x20,0x40,0x80,0x80,0x00},
    ['$'] = {0x20,0x78,0xA0,0x70,0x28,0xF8,0x20,0x00},
    ['~'] = {0x40,0xA0,0x00,0x00,0x00,0x00,0x00,0x00}, // Tilde sederhana
    ['?'] = {0x70,0x88,0x08,0x30,0x20,0x00,0x20,0x00},  // Question mark
    ['@'] = {0x70,0x88,0x98,0xA8,0x90,0x80,0x70,0x00},
    ['-'] = {0x00,0x00,0x00,0xF0,0x00,0x00,0x00,0x00},
    ['_'] = {0x00,0x00,0x00,0x00,0x00,0x00,0xF8,0x00},
    ['='] = {0x00,0x00,0xF8,0x00,0xF8,0x00,0x00,0x00},
    ['+'] = {0x00,0x20,0x20,0xF8,0x20,0x20,0x00,0x00},
    ['*'] = {0x00,0x50,0x20,0xF8,0x20,0x50,0x00,0x00},
    ['#'] = {0x50,0x50,0xF8,0x50,0xF8,0x50,0x50,0x00},
    ['!'] = {0x20,0x20,0x20,0x20,0x20,0x00,0x20,0x00},
    [','] = {0x00,0x00,0x00,0x00,0x00,0x30,0x20,0x40},
    [';'] = {0x00,0x30,0x00,0x00,0x30,0x20,0x40,0x00},
    ['('] = {0x10,0x20,0x40,0x40,0x40,0x20,0x10,0x00},
    [')'] = {0x40,0x20,0x10,0x10,0x10,0x20,0x40,0x00},
    ['['] = {0x70,0x40,0x40,0x40,0x40,0x40,0x70,0x00},
    [']'] = {0x70,0x10,0x10,0x10,0x10,0x10,0x70,0x00},
    ['{'] = {0x18,0x20,0x20,0x40,0x20,0x20,0x18,0x00},
    ['}'] = {0xC0,0x20,0x20,0x10,0x20,0x20,0xC0,0x00},
    ['<'] = {0x08,0x10,0x20,0x40,0x20,0x10,0x08,0x00},
    ['>'] = {0x80,0x40,0x20,0x10,0x20,0x40,0x80,0x00},
    ['a'] = {0x00,0x00,0x70,0x08,0x78,0x88,0x78,0x00},
    ['e'] = {0x00,0x00,0x70,0x88,0xF0,0x80,0x70,0x00},
    ['i'] = {0x20,0x00,0x60,0x20,0x20,0x20,0x70,0x00},
    ['l'] = {0x60,0x20,0x20,0x20,0x20,0x20,0x70,0x00},
    ['m'] = {0x00,0x00,0xD0,0xA8,0xA8,0xA8,0xA8,0x00},
    ['n'] = {0x00,0x00,0xB0,0xC8,0x88,0x88,0x88,0x00},
    ['o'] = {0x00,0x00,0x70,0x88,0x88,0x88,0x70,0x00},
    ['r'] = {0x00,0x00,0xB0,0xC8,0x80,0x80,0x80,0x00},
    ['s'] = {0x00,0x00,0x78,0x80,0x70,0x08,0xF0,0x00},
    ['u'] = {0x00,0x00,0x88,0x88,0x88,0x98,0x68,0x00}
    // Isi karakter lain sesuai kebutuhan
};
// Untuk karakter yang tidak ada di atas, akan muncul sebagai blok kosong atau karakter terakhir yang valid
// jika tidak ada penanganan khusus di vga_draw_char_kernel.